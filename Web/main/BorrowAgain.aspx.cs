...
namespace Web.main
{
    ...
    /// 书本续借
    /// </summary>
    public partial class BorrowAgain : System.Web.UI.Page
    {
        ...
        tb_borrow_again borrowAgainBLL = new tb_borrow_again();
        ...
        web.Model.tb_borrow_again borrowAgainModel = null;
            ...
            try
            {
                ...
                if (ok)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (card.stop == false)
                        {
                            Response.Write("<script type='text/javascript'>alert(\"对不起，您不能续借，因为您的借阅卡处于停借状态，请联系管理员。\")</script>");
                        }
                        //借阅卡：挂失。现在不能借书。
                        else if (card.lose == false)
                        {
                            Response.Write("<script type='text/javascript'>alert(\"对不起，您不能续借，因为您的借阅卡处于挂失状态，请联系管理员。\")</script>");
                        }
                        else
                        {
                            //将归还日期 加上60天
                            borrow.again_count += 1;
                            //更新
                            bookBorrowBLL.Update(borrow);

                            //创建续借记录
                            borrowAgainModel = new web.Model.tb_borrow_again();
                            borrowAgainModel.again_date = DateTime.Now;
                            borrowAgainModel.again_intime= ((DateTime)borrow.in_time).AddDays(60);
                            borrowAgainModel.book_borrow_id = borrow.id;
                            borrowAgainModel.manager = Session["una"].ToString();
                            borrowAgainBLL.Add(borrowAgainModel);
